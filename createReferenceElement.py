        ...
        if 3 == nOfElementNodes:
            ...
            if 6 == nOfElementNodes:
                ...
                if 10 == nOfElementNodes:
                    ...
                    innerNodes = 10
                    faceNodes1d = np.arange(1,4+1)
                else:
                    if 15 == nOfElementNodes:
                        nDeg = 4
                        faceNodes = np.array([[1,4,5,6,2],[2,7,8,9,3],[3,10,11,12,1]])
                        innerNodes = np.arange(13,15+1)
                        faceNodes1d = np.arange(1,5+1)
                    else:
                        if 21 == nOfElementNodes:
                            nDeg = 5
                            faceNodes = np.array([[1,np.arange(4,7+1),2],[2,np.arange(8,11+1),3],[3,np.arange(12,15+1),1]])
                            innerNodes = np.arange(16,21+1)
                            faceNodes1d = np.arange(1,6+1)
                        else:
                            if 28 == nOfElementNodes:
                                nDeg = 6
                                faceNodes = np.array([[1,np.arange(4,8+1),2],[2,np.arange(9,13+1),3],[3,np.arange(14,18+1),1]])
                                innerNodes = np.arange(19,28+1)
                                faceNodes1d = np.arange(1,7+1)
                            else:
                                if 36 == nOfElementNodes:
                                    nDeg = 7
                                    faceNodes = np.array([[1,np.arange(4,9+1),2],[2,np.arange(10,15+1),3],[3,np.arange(16,21+1),1]])
                                    innerNodes = np.arange(22,36+1)
                                    faceNodes1d = np.arange(1,8+1)
                                else:
                                    if 45 == nOfElementNodes:
                                        nDeg = 8
                                        faceNodes = np.array([[1,np.arange(4,10+1),2],[2,np.arange(11,17+1),3],[3,np.arange(18,24+1),1]])
                                        innerNodes = np.arange(25,45+1)
                                        faceNodes1d = np.arange(1,9+1)
                                    else:
                                        if 55 == nOfElementNodes:
                                            nDeg = 9
                                            faceNodes = np.array([[1,np.arange(4,11+1),2],[2,np.arange(12,19+1),3],[3,np.arange(20,27+1),1]])
                                            innerNodes = np.arange(28,55+1)
                                            faceNodes1d = np.arange(1,10+1)
                                        else:
                                            if 66 == nOfElementNodes:
                                                nDeg = 10
                                                faceNodes = np.array([[1,np.arange(4,12+1),2],[2,np.arange(13,21+1),3],[3,np.arange(22,30+1),1]])
                                                innerNodes = np.arange(31,66+1)
                                                faceNodes1d = np.arange(1,11+1)
                                            else:
                                                if 78 == nOfElementNodes:
                                                    nDeg = 11
                                                    faceNodes = np.array([[1,np.arange(4,13+1),2],[2,np.arange(14,23+1),3],[3,np.arange(24,33+1),1]])
                                                    innerNodes = np.arange(34,78+1)
                                                    faceNodes1d = np.arange(1,12+1)
                                                else:
            ...
            if 4 == nOfElementNodes:
                ...
                if 9 == nOfElementNodes:
                    ...
                    if 16 == nOfElementNodes:
                        ...
                        innerNodes = np.arange(13,16+1)
                        faceNodes1d = np.arange(1,4+1)
                        coord1d = feketeNodes1D(nDeg,faceNodes1d)
                        coord2d = np.zeros(((nDeg + 1) ** 2,2))
                        for i in np.arange(1,nDeg + 1+1).reshape(-1):
                            ind = (i - 1) * (nDeg + 1) + (np.arange(1,(nDeg + 1)+1))
